...
class Map extends Component {
  ...
  refreshDevices = () => {
    request.get(
      { url: "http://localhost:49215/get_devices" },
      (err, res, body) => {
        if (err) console.log(err);
          ...
          try {
            ...
            if (devices.length) {
              ...
              toast.error(
                "No devices found, make sure you trusted the computer on your phone.",
                errorOptions
              );
            }
          ...
          request.post(
            {
              ...
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                udid: this.state.devices[0].udid,
                lat,
                lng,
              }),
            },
              ...
              if (err) console.log(err);
                ...
                if (result.error) {
                  if (result.error === "Unable to mount developer image.") {
                    toast.error(
                      "Unable to mount developer image, please make sure the device is unlocked.",
                      errorOptions
                    );
                  } else {
                  ...
                  toast.success(
                    `Successfully set device's location`,
                    successOptions
                  );
                }
              }
            ...
            (err, res, body) => {
              if (err) console.log(err);
                ...
                if (result.error) {
                  ...
                  toast.success(
                    `Location spoof has stopped, if your location is stuck please restart your device.`,
                    successOptions
                  );
                }
              }
      ...
      <div className="Map">
        ...
        <ul className="phoneList">
          {this.state.devices.map((device, key) => (
            <li key={key}>
              <span className="icon">
                <FontAwesomeIcon icon={faCircle} style={{ color: "lime" }} />
              </span>
          ...
          <li style={{ cursor: "pointer" }} onClick={this.refreshDevices}>
            <span>Refresh Phone List</span>
          </li>
        </ul>
        <Mapbox
        ...
        <div className="buttons">
          <button
            disabled={this.state.setLocation.disabled}
            id="setLocation"
            onClick={this.setPhoneLocation}
          >
            Set Location
          </button>
          <button
            disabled={this.state.setLocation.disabled}
            id="stopLocation"
            onClick={this.stopPhoneLocation}
          >
            Stop Location
          </button>
        </div>
